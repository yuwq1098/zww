(function(w){
    ...
    // 随机生成成绩
    const randomScore = (start = 0, end = 100) => {
        let num = Math.ceil(Math.random() * (end - start)) + start
     ...
     * 随机生成无序的数组
     * @param {Number} n 
     * @param {Number} swapTimes 
     */
    const generateRandomOrderedArray = (n) => {
        let arr = []
        for(let i = 0; i < n; i++){
            let res = Math.ceil(Math.random() * n) + Math.ceil(Math.random() * n);
            arr[i] = res
        }
        return arr
    }
    ...
    const generateNearlyOrderedArray = (n, swapTimes) => {
        ...
        for(let i = 0; i< swapTimes; i++){
            let posx = Math.ceil(Math.random() * n);
            let posy = Math.ceil(Math.random() * n);
            let [a, b] = [arr[posx], arr[posy]]
            arr[posx] = b
            arr[posy] = a
        }
        return arr
     ...
     * @param {Boolean} simple 是否单姓
     * @param {Number} len 名字长度
     */
    const randomName = ( random = false, simple = true, len = 2) => {
        if (random) {
            simple = Math.ceil(Math.random() * 2) > 1
            let rd_len = Math.ceil(Math.random() * 2)
            len = simple ? rd_len + 1 : rd_len + 2
        } 
        // 单姓
        let surName = [
        ...
        // 双字姓
        let doubleSurName = ["万俟","司马","上官","欧阳","夏侯","诸葛","闻人","东方",
        ...
        // 中文字
        let word = ["一","乙","二","十","丁","厂","七","卜","人","入","八","九","几","儿","了","力","乃","刀","又",
        ...
        let str = ""
        simple ? str += (surName[Math.floor(Math.random() * surNameLen)]) : str += (doubleSurName[Math.floor(Math.random() * doubleSurNameLen)]);
        for (let i = 0; i < len - str.length; i++) {
            str += (word[Math.floor(Math.random() * wordLen)]);
        }
        return str;
    }
    ...
    /**
     * 性能测试
     * @param {String} name 排序方法名称
     * @param {Function} fn 排序方法体
     * @param {Array} data 测试用例
     */
    const perfTest = (name, fn, data) => {
